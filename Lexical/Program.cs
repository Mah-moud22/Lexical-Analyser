...
namespace Lexical
{
        ...
        public void Multi(ref string file)
        {
            ...
            {    //detect is not in string
                ...
                if (i + 1 < file.Length && file[i] == '/' && file[i + 1] == '*' && isstring)
                {
                    ...
                    string newstring = file.Remove(start, cnt + 2);
                    file = newstring;
                    i -= cnt;
                    if (i < 0) i = 0;
                }
    ...
    public class Program 
    {
        ...
        {
            // To check if the elements had been inserted
            if(Keywords.Count == 0)
            {
                insertKeywords();
                insertOperators();
            ...
            for (int i = 0; i < file.Length; i++)
            {
                    ...
                    Spaces.ContainsKey(file[i].ToString()))
                {
                    ...
                    if ((file[i] == '+' || file[i] == '-') && (file[i - 1] == 'e' || file[i - 1] == 'E'))
                    {
                        int index = i - 2;
                    ...
                    else if (Operators.ContainsKey(file[i].ToString()))
                    {
                        ...
                        else if (i + 1 < file.Length && Operators.ContainsKey(file[i + 1].ToString()))
                        {
                            KeyValuePair<string, string> item = new KeyValuePair<string, string>(file[i].ToString() + file[i + 1], "Operator");
                            LexicalLexeme.Add(item);
                            res += file[i] + file[i+1] + "\t\t\t\t Operator\r\n";
                            Console.WriteLine("\n" + file[i] + file[i + 1] + "--------> Operator\n");
                            i++;
                        }
                        else
                        {
